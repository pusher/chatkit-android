...
internal data class AuxiliaryState(
    val version: Int,
    val currentUserReceived: LastChange<CurrentUserReceived>?,
    ...
    fun with(action: Action) =
        when (action) {
            is CurrentUserReceived ->
                copy(nextVersion, currentUserReceived = LastChange(nextVersion, action))
